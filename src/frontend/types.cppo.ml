...
and syntactic_value =
  ...
  | Context      of syntactic_value input_context
...
and 'a input_context =
  HorzBox.context_main * 'a context_sub

and 'a context_sub = {
  math_command : 'a math_command_func;
  code_text_command : 'a code_text_command_func;
  dummy : unit;
}

and 'a math_command_func =
  | MathCommand of 'a
      (* (input_context -> math list -> HorzBox.horz_box list) *)

and 'a code_text_command_func =
  | DefaultCodeTextCommand
  | CodeTextCommand of 'a
...
and math_element =
  | MathElement           of HorzBox.math_kind * 'a math_element_main
