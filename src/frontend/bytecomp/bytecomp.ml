...
let compile_and_exec_0 (env : environment) (ast : abstract_tree) : compiled_value * compiled_environment option =
...
let compile_and_exec_1 (env : environment) (ast : abstract_tree) : code_value * compiled_environment option =
  let (ir, env) = Ir.transform_ast_1 env ast in
  let instrs = Compiler.compile ir [] in
  let (value, envopt) = Vm.exec_code env instrs in
  match value with
  | CVCodeValue(cv) -> (cv, envopt)
  | _               -> failwith "TODO (error)" (* report_bug_value "compile_and_exec_1: not a CodeValue(...)" value *)
