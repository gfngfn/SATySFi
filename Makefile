...
EXTERNAL=external
OCB_FLAGS = -cflags -unsafe-string -cflag -w -cflag -3 -use-ocamlfind -use-menhir -I $(SRCROOT)/ -I $(FRONTEND)/ -I $(BACKEND)/ -I $(CHARDECODER)/ -I $(EXTERNAL)/otfm/src/ -I $(EXTERNAL)/camlpdf/ -I $(EXTERNAL)/ucorelib/src/ -pkgs "ppx_deriving.show,core_kernel,result,uutf,bitv,batteries,menhirLib,yojson,camlimages,camlimages.jpeg" -tag thread -lflags "flatestubs.c rijndael-alg-fst.c stubs-aes.c sha2.c stubs-sha2.c" -yaccflags "--table --explain"
TARGET=satysfi
