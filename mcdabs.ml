...
module McdAbs : sig
  ...
  let rec concrete_to_abstract conctr =
    ...
    | NonTerminal(ListBySep, (NonTerminal(Block, blk)) :: lstbysp) -> (
          match lstbysp with
            [] -> (
              	print_string "#ListBySep []" ; print_newline () ;
            	  concrete_to_abstract (NonTerminal(Block, blk))
            	)
          | _ -> (
              	print_string "#ListBySep _" ; print_newline () ;
          	    Separated(concrete_to_abstract (NonTerminal(Block, blk)), concrete_to_abstract (NonTerminal(Block, lstbysp)))
          	  )
        )
  ...
  (*    | [Terminal(MACROWID); Terminal(CTRLSEQ(f)); args; grp1; grp2]
    ...
    | _ -> (report_error "illegal concrete tree" ; Invalid)
